	...
	*/
			...
			if ((eachName.compareToIgnoreCase(ALL) == 0) && (in.indexOf(ALL) != -1)) {
				...
				while(keysPeople.hasMoreElements()){
					...
					if ((idx == _FR) && (m_bClearing == false)) {
						person.m_amount[person.IND_PAID] += iAmt ;
				...
				while(keysPeople.hasMoreElements()){
					...
					if ((idx == _FR) && (m_bClearing == false)) {
						person.m_amount[person.IND_PAID] += rAmt ;
				...
				try {
					...
					if ((idx == _FR) && (m_bClearing == false)) {
						person.m_amount[person.IND_PAID] += pAmt ;
	...
	private void prepareToExportGroup(String item, String category, String vendor, String desc, String amt, String from, String to, String group, String action, String def)
	{
		...
		*/
		...
		while (iter.hasNext()) {
			...
			sIndPaid += ITEM_SEPARATOR + lBr + person.m_name + AMT_INDICATOR + roundAmount(person.m_amount[person.IND_PAID]) + rBr ;
		...
		sIndPaid = lPAD + sIndPaid.substring(ITEM_SEPARATOR.length(), sIndPaid.length()) + rPAD ;
		//System.out.println("prepareToExportGroup::sIndPaid = " + sIndPaid);

		aLine += TAB_SEPARATOR + sTransAmt + TAB_SEPARATOR + sSysAmt /*+ TAB_SEPARATOR*/ + sPerAmt + roundAmount(cs)+ TAB_SEPARATOR + sIndAmt + roundAmount(indcs) + TAB_SEPARATOR + sIndPaid  ;
	...
	private void exportToCSV(String fileName, String group)
	{
		...
		try {
			...
			sHeader03 = H_INDIVIDUAL_TOTALS + padHeader(aGroup.size() - 1 + 1 /*IndCheckSum*/) + TAB_SEPARATOR + H_INDIVIDUAL_PAID ;
			...
			if (m_bSys == true)
				sHeader += TAB_SEPARATOR  + sPersons + TAB_SEPARATOR + SYS  + TAB_SEPARATOR + sPersons + TAB_SEPARATOR + H_CHECKSUM + TAB_SEPARATOR + sPersons + TAB_SEPARATOR + H_INDCHECKSUM + TAB_SEPARATOR + sPersons;
			else
				sHeader += TAB_SEPARATOR /*SYS + TAB_SEPARATOR*/ + sPersons + TAB_SEPARATOR + sPersons + TAB_SEPARATOR + H_CHECKSUM + TAB_SEPARATOR + sPersons + TAB_SEPARATOR + H_INDCHECKSUM + TAB_SEPARATOR + sPersons ;
			...
			for (String aLine : exportLines) {
				...
				//individual paid
				int f3 = aLine.indexOf(lPAD, t21 + rPAD.length()) + lPAD.length() ;
				int t3 = aLine.indexOf(rPAD, f3) ;
				...
				*/
				...
				outLine += TAB_SEPARATOR + sNewLine2 + indcheckSum ;
				outLine += TAB_SEPARATOR + sNewLine3 ;
				sToFile = outLine ;
	...
	private void dumpCollection()
	{
		...
		while(keysGroup.hasMoreElements()){
			...
			while(keysPeople.hasMoreElements()){
				...
				sIndPaid += lBr + person.m_name + AMT_INDICATOR + roundAmount(person.m_amount[person.IND_PAID]) + rBr ;
				System.out.println(person.m_name + ":" + sTransAmt + DUMP_SEPARATOR + sPerAmt + DUMP_SEPARATOR + sIndAmt + DUMP_SEPARATOR + sIndPaid);
			}
		}
		System.out.println("--------------------------------------");
	}
