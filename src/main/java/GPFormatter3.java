...
public class GPFormatter3 {
	...
	public void prepareToExportGroup(String item, String category, String vendor, String desc, String amt, String from, String to, String group, String action, String def)
	{
		...
		*/
		...
		Hashtable<String, Person3> aGroup = Utils.m_GroupCollection3.get(group) ;
		List<String> mapKeys = new ArrayList<String>(aGroup.keySet());
		Collections.sort(mapKeys);
	    iter = mapKeys.iterator();
		while (iter.hasNext()) {
			Person3 person = aGroup.get(iter.next());
			sTransAmt += Utils._ITEM_SEPARATOR + Utils.lBr + person.m_name + Utils._AMT_INDICATOR + Utils.roundAmount(person.m_amount.get(Person3.AccountEntry.TRANS_AMT)) + Utils.rBr ;
			sPerAmt += Utils._ITEM_SEPARATOR + Utils.lBr + person.m_name + Utils._AMT_INDICATOR + Utils.roundAmount(person.m_amount.get(Person3.AccountEntry.SYS_SUM)) + Utils.rBr ;
			sIndAmt += Utils._ITEM_SEPARATOR + Utils.lBr + person.m_name + Utils._AMT_INDICATOR + Utils.roundAmount(person.m_amount.get(Person3.AccountEntry.IND_SUM)) + Utils.rBr ;
			sIndPaid += Utils._ITEM_SEPARATOR + Utils.lBr + person.m_name + Utils._AMT_INDICATOR + Utils.roundAmount(person.m_amount.get(Person3.AccountEntry.IND_PAID)) + Utils.rBr ;
			cs = person.m_amount.get(Person3.AccountEntry.CHK_SUM) ;
			indcs = person.m_amount.get(Person3.AccountEntry.CHK_INDSUM) ;
		}
